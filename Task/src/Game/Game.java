...
public class Game {
    /*public static void play(GameData data, Player firstPlayer){
        Player tmpPlayer = firstPlayer;
        ShowSevice.printGameField(data);
        while (true){
            //Если гусей не осталось то гусь проиграл
            Point position = AuxiliaryService.getPosition();
            Cell lastCell = AuxiliaryService.getCellByIndex(data, position);

            Map<Cell,Direction> options = Options.valueOf(data.getPlayerGameSide().get(tmpPlayer)).getOptions(data, tmpPlayer, data.getPositionShapes().get(lastCell), lastCell);

            //Проверка если нет вариантов у волка то он проиграл

            Set<Cell> cells = options.keySet();

            //ShowSevice.printGameField(data, cells);

            position = AuxiliaryService.getPosition();
            Cell newCell = AuxiliaryService.getCellByIndex(data, position);
            while (newCell == null || !cells.contains(newCell)) {
                System.out.println("Вы не можете туда двигаться");
                position = AuxiliaryService.getPosition();
                newCell = AuxiliaryService.getCellByIndex(data, position);
            }

            tmpPlayer = RepositionService.reposition(data, tmpPlayer, data.getPositionShapes().get(lastCell), lastCell, newCell, options.get(newCell));
            ShowSevice.printGameField(data);
